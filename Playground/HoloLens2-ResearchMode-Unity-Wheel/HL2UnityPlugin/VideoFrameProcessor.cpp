...
{        
    ...
    OutputDebugString(L"Executing Dumping Frames");
    auto spMemoryBufferByteAccess{ bitmapBuffer.CreateReference().as<::Windows::Foundation::IMemoryBufferByteAccess>() };
    winrt::check_hresult(spMemoryBufferByteAccess->GetBuffer(&m_videoFrameBuffer, &m_videoFrameBufferLength));
    isVideoFrameReady = true;
    /*if (!m_videoFrameBuffer) {
        m_videoFrameBuffer = new UINT8[pixelBufferDataLength];
    }
    memcpy(m_videoFrameBuffer, pixelBufferData, pixelBufferDataLength);
    m_videoFrameBufferLength = pixelBufferDataLength;*/

    //m_tarball->AddFile(bitmapPath, &pixelBufferData[0], pixelBufferDataLength);    
}

UINT8* VideoFrameProcessor::ReadVideoFrame() {
    isReadFinished = false;
    if (isVideoFrameReady && isReadFinished) {
        return &m_videoFrameBuffer[0];
    }
    else {
        UINT8 buffer[1] = { 0 };
        return &buffer[0];
...
UINT32 VideoFrameProcessor::GetPVBufferLength() {
    if (isVideoFrameReady) {
        return m_videoFrameBufferLength;
    }
    else {
        return 0;
    }
...
bool VideoFrameProcessor::CheckVideoFrame() {
    return isVideoFrameReady;
}

void VideoFrameProcessor::okayToProceed() {
    isReadFinished = true;
}
...
void VideoFrameProcessor::CameraWriteThread(VideoFrameProcessor* pProcessor)
{
    ...
    {
        if (!pProcessor->isReadFinished) {
            continue; //We will not write new file if reading is not finished
        }
