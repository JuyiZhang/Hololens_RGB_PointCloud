...
namespace winrt::HL2UnityPlugin::implementation
{
    ...
    winrt::Windows::Foundation::IAsyncAction HL2ResearchMode::InitializeVideoFrameProcessorAsync() {
    ...
    bool HL2ResearchMode::IsVideoFrameProcessorWantedAndReady(HL2ResearchMode* pHL2ResearchMode)
    {
        return (pHL2ResearchMode->m_videoFrameProcessorOperation == nullptr ||
            pHL2ResearchMode->m_videoFrameProcessorOperation.Status() == winrt::Windows::Foundation::AsyncStatus::Completed);
    }
    ...
    void HL2ResearchMode::DepthSensorLoop(HL2ResearchMode* pHL2ResearchMode)
    {
        ...
        if (!pHL2ResearchMode->m_depthSensorLoopStarted)
        {
            ...
            if (pHL2ResearchMode->m_videoFrameProcessor) {
                pHL2ResearchMode->m_videoFrameProcessor->StartRecording();
            }
        }
        else return;
        ...
        try 
        {
            ...
            {
                /*if (pHL2ResearchMode->IsVideoFrameProcessorWantedAndReady(pHL2ResearchMode)) {
                    UINT32 outPVTextureBufferSize = pHL2ResearchMode->m_videoFrameProcessor->GetPVBufferLength();
                    UINT8* outVideoFrame = pHL2ResearchMode->m_videoFrameProcessor->ReadVideoFrame();
                    char PVBufferLengthString[80];
                    sprintf_s(PVBufferLengthString, "The Length of PV Buffer is %d", (int)outPVTextureBufferSize);
                    OutputDebugStringA(PVBufferLengthString);
                    if (!pHL2ResearchMode->m_PVTexture) {
                        OutputDebugString(L"Create space for PV Texture...\n");
                        pHL2ResearchMode->m_PVTexture = new UINT8[outPVTextureBufferSize];
                    }
                    pHL2ResearchMode->m_PVTexture = pHL2ResearchMode->m_videoFrameProcessor->ReadVideoFrame();
                    pHL2ResearchMode->m_PVTextureBufferSize = pHL2ResearchMode->m_videoFrameProcessor->GetPVBufferLength();
                    pHL2ResearchMode->m_PVTextureUpdated = true;
                }*/
    ...
    inline int HL2ResearchMode::GetPVTextureBufferSize() { return m_PVTextureBufferSize; }

    inline bool HL2ResearchMode::DepthMapTextureUpdated() { return m_depthMapTextureUpdated; }

    inline bool HL2ResearchMode::PVTextureUpdated() { return m_PVTextureUpdated; }
    ...
    com_array<uint8_t> HL2ResearchMode::GetPVTextureBuffer() {
        std::lock_guard<std::mutex> l(mu);

        if (this->IsVideoFrameProcessorWantedAndReady(this)) {
            /*UINT32 outPVTextureBufferSize = pHL2ResearchMode->m_videoFrameProcessor->GetPVBufferLength();
            UINT8* outVideoFrame = pHL2ResearchMode->m_videoFrameProcessor->ReadVideoFrame();
            char PVBufferLengthString[80];
            sprintf_s(PVBufferLengthString, "The Length of PV Buffer is %d", (int)outPVTextureBufferSize);
            ...
            const UINT8* pPVTexture = this->m_videoFrameProcessor->ReadVideoFrame();
            const UINT32 pPVTextureBufferSize = this->m_videoFrameProcessor->GetPVBufferLength();
            //UINT8* outputTexture = new UINT8[pPVTextureBufferSize];
            //memcpy(outputTexture, pPVTexture, pPVTextureBufferSize);
            ...
            if (!this->m_videoFrameProcessor->CheckVideoFrame()) {
                this->m_videoFrameProcessor->okayToProceed();
                return com_array<uint8_t>();
                
            }
            else {
                com_array<byte> tempBuffer = com_array<byte>(pPVTexture, pPVTexture + pPVTextureBufferSize);
                this->m_videoFrameProcessor->okayToProceed();
                return tempBuffer;
            }
        }
